...
export default function AdminNovoQuizPage() {
  const router = useRouter();
  const { data: coursesData, isLoading: coursesLoading } = useCoursesAdmin({ perPage: 200 });
  ...
  async function handleSave(publish: boolean) {
    ...
    try {
      ...
      setTimeout(() => router.push("/quizzes"), 800);
    } catch (err: unknown) {
