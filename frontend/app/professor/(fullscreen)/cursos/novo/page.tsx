...
// TODO: integrar com POST /professor/courses
...
const CATEGORIES = ["Segurança", "Técnico", "Compliance", "Comportamental", "Liderança"];
    ...
    <div ref={setNodeRef} style={{ ...style, background: "#fff", border: "1px solid #ece4e4", borderRadius: 14, overflow: "hidden" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "15px 16px", background: "#fcfafa", borderBottom: "1px solid #f4eded" }}>
        ...
        <button
          ...
          type="button"
          style={{ width: 30, height: 30, border: "none", background: "transparent", cursor: "pointer", color: "#a89e9c", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}
        >
          <Trash2 size={15} />
        </button>
      </div>
      <div style={{ padding: "10px 12px", display: "flex", flexDirection: "column", gap: 8 }}>
        {module.lessons.map((l) => (
          <div key={l.id} style={{ display: "flex", alignItems: "center", gap: 11, padding: "10px 12px", border: "1px solid #f0e8e8", borderRadius: 10, background: "#fff" }}>
            <span style={{ cursor: "grab", color: "#cfc4c2", display: "flex", flexShrink: 0 }}>
            ...
            <button
              onClick={() => onDeleteLesson(module.id, l.id)}
              type="button"
              style={{ width: 26, height: 26, border: "none", background: "transparent", cursor: "pointer", color: "#c98a8a", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}
            >
        ...
        <button
          ...
          type="button"
          style={{ display: "inline-flex", alignItems: "center", gap: 8, fontFamily: "Manrope, sans-serif", fontSize: 13.5, fontWeight: 700, color: PRIMARY, background: "#fff", border: "1.5px dashed #e2d2d2", borderRadius: 10, padding: "11px 14px", cursor: "pointer", alignSelf: "flex-start" }}
        >
...
export default function CriarCursoPage() {
  ...
  const [courseTitle, setCourseTitle] = useState("Segurança no Trabalho");
  const [description, setDescription] = useState("");
  const [category, setCategory] = useState("Segurança");
  const [level, setLevel] = useState<CourseLevel>("inter");
  const [objectives, setObjectives] = useState(["Identificar riscos no ambiente de trabalho", "Aplicar normas de segurança (NRs)", "Usar corretamente os EPIs"]);

  const [modules, setModules] = useState<ModuleItem[]>([
    { id: `${uid}-m1`, title: "Fundamentos da Segurança", lessons: [
      { id: `${uid}-l1`, title: "Boas-vindas e objetivos", type: "video" },
      { id: `${uid}-l2`, title: "O que são as NRs", type: "video" },
      { id: `${uid}-l3`, title: "Quiz: conceitos iniciais", type: "quiz" },
    ]},
    { id: `${uid}-m2`, title: "EPIs e Prevenção", lessons: [
      { id: `${uid}-l4`, title: "Tipos de EPI", type: "video" },
      { id: `${uid}-l5`, title: "Material de apoio (PDF)", type: "file" },
    ]},
  ]);

  const [download, setDownload] = useState(true);
  const [cert, setCert] = useState(true);
  const [minScore] = useState("7");
  ...
  function addModule() {
    const id = `m${Date.now()}`;
    setModules((prev) => [...prev, { id, title: "Novo módulo", lessons: [] }]);
  }

  function addLesson(moduleId: string) {
    setModules((prev) => prev.map((m) => {
      if (m.id !== moduleId) return m;
      return { ...m, lessons: [...m.lessons, { id: `l${Date.now()}`, title: "Nova aula", type: "video" }] };
    }));
  }
    ...
    <div style={{ minHeight: "100vh", background: "#f6f4f3", display: "flex", flexDirection: "column" }}>
      ...
      <header style={{ background: "#fff", borderBottom: "1px solid #ece4e4", padding: "14px clamp(16px,3vw,32px)", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 16, position: "sticky", top: 0, zIndex: 20 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 16, minWidth: 0 }}>
          <button
            onClick={() => router.back()}
            style={{ display: "inline-flex", alignItems: "center", gap: 8, fontFamily: "inherit", fontSize: 13.5, fontWeight: 700, color: "#6a605e", textDecoration: "none", background: "#f6f1f1", border: "1px solid #ece4e4", padding: "9px 14px", borderRadius: 10, cursor: "pointer", flexShrink: 0 }}
          >
          ...
          <div style={{ minWidth: 0 }}>
            <div style={{ fontSize: 15, fontWeight: 800, letterSpacing: "-0.01em", color: "#16100f", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{courseTitle || "Curso sem título"}</div>
            <div style={{ fontSize: 12, fontWeight: 600, color: "#a89e9c" }}>Rascunho · salvo automaticamente</div>
          </div>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 10, flexShrink: 0 }}>
          <button style={{ fontFamily: "inherit", fontSize: 13.5, fontWeight: 700, color: "#16100f", background: "#fff", border: "1.5px solid #e2d9d9", borderRadius: 10, padding: "10px 16px", cursor: "pointer" }}>
            Salvar rascunho
          </button>
          <button style={{ display: "inline-flex", alignItems: "center", gap: 7, fontFamily: "inherit", fontSize: 13.5, fontWeight: 800, color: "#fff", background: PRIMARY, border: "none", borderRadius: 10, padding: "11px 18px", cursor: "pointer", boxShadow: "0 6px 16px rgba(204,31,31,0.26)" }}>
          ...
          {step === 0 && (
            <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
              ...
              <div style={{ background: "#fff", border: "1px solid #ece4e4", borderRadius: 16, padding: 26, display: "flex", flexDirection: "column", gap: 18 }}>
                ...
                <div>
                  <label style={labelS}>Thumbnail</label>
                  <div style={{ border: "1.5px dashed #e2d2d2", borderRadius: 12, padding: 22, display: "flex", alignItems: "center", gap: 16, background: "#faf7f7", cursor: "pointer" }}>
                    <div style={{ width: 48, height: 48, borderRadius: 11, background: "#fceeee", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                      <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke={PRIMARY} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="3" width="18" height="18" rx="2"/><circle cx="9" cy="9" r="2"/><path d="m21 15-3.6-3.6a2 2 0 0 0-2.8 0L6 20"/></svg>
                    </div>
                    <div>
                      <div style={{ fontSize: 14, fontWeight: 700, color: "#3a3030" }}>Arraste uma imagem ou <span style={{ color: PRIMARY }}>procure</span></div>
                      <div style={{ fontSize: 12, fontWeight: 500, color: "#a89e9c", marginTop: 2 }}>PNG ou JPG · 1280×720</div>
                    </div>
                  </div>
                </div>
                <div>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 9 }}>
                    {objectives.map((o, i) => (
                      <div key={i} style={{ display: "flex", alignItems: "center", gap: 10, background: "#faf7f7", border: "1px solid #eadfdf", borderRadius: 10, padding: "11px 13px" }}>
                        ...
                        <span style={{ flex: 1, fontSize: 14, fontWeight: 500, color: "#3a3030" }}>{o}</span>
                    ...
                    <button
                      onClick={() => setObjectives((prev) => [...prev, "Novo objetivo de aprendizado"])}
                      type="button"
                      style={{ display: "inline-flex", alignItems: "center", gap: 8, fontFamily: "Manrope, sans-serif", fontSize: 13.5, fontWeight: 700, color: PRIMARY, background: "#fff", border: "1.5px dashed #e2d2d2", borderRadius: 10, padding: "11px 14px", cursor: "pointer", alignSelf: "flex-start" }}
                    >
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
              <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", gap: 16, flexWrap: "wrap" }}>
                ...
                <button
                  onClick={addModule}
                  type="button"
                  style={{ display: "inline-flex", alignItems: "center", gap: 8, fontFamily: "inherit", fontSize: 14, fontWeight: 700, color: "#fff", background: PRIMARY, border: "none", padding: "11px 18px", borderRadius: 10, cursor: "pointer", boxShadow: "0 6px 16px rgba(204,31,31,0.26)" }}
                >
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
              ...
              <div style={{ background: "#fff", border: "1px solid #ece4e4", borderRadius: 16, padding: "10px 26px" }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20, padding: "20px 0" }}>
                  <div>
                    <label style={labelS}>Nota mínima no quiz (aprovação)</label>
                    <input type="number" defaultValue={minScore} min="0" max="10" step="0.5" style={{ ...inputS, maxWidth: 160 }} />
                  </div>
                  <div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
              ...
              <div style={{ background: "#fff", border: "1px solid #ece4e4", borderRadius: 16, overflow: "hidden" }}>
                <div style={{ height: 120, background: `linear-gradient(135deg,${PRIMARY},#e85a4f)`, display: "flex", alignItems: "center", padding: "0 26px" }}>
                  <div>
                    <div style={{ fontSize: 11.5, fontWeight: 800, letterSpacing: "0.05em", textTransform: "uppercase", color: "rgba(255,255,255,0.85)" }}>{category} · {level === "basico" ? "Básico" : level === "inter" ? "Intermediário" : "Avançado"}</div>
              ...
              <div style={{ display: "flex", gap: 12, justifyContent: "flex-end" }}>
                <button style={{ fontFamily: "inherit", fontSize: 14.5, fontWeight: 700, color: "#16100f", background: "#fff", border: "1.5px solid #e2d9d9", borderRadius: 11, padding: "13px 22px", cursor: "pointer" }}>
                  Salvar rascunho
                </button>
                <button
                  onClick={() => router.push("/professor/cursos")}
                  style={{ display: "inline-flex", alignItems: "center", gap: 8, fontFamily: "inherit", fontSize: 14.5, fontWeight: 800, color: "#fff", background: PRIMARY, border: "none", borderRadius: 11, padding: "13px 24px", cursor: "pointer", boxShadow: "0 10px 24px rgba(204,31,31,0.26)" }}
                >
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginTop: 26 }}>
            {step > 0 ? (
              <button
                ...
                type="button"
                style={{ display: "inline-flex", alignItems: "center", gap: 8, fontFamily: "inherit", fontSize: 14.5, fontWeight: 700, color: "#16100f", background: "#fff", border: "1.5px solid #e2d9d9", borderRadius: 11, padding: "13px 22px", cursor: "pointer" }}
              >
                <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="#6a605e" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><path d="M19 12H5"/><path d="m12 19-7-7 7-7"/></svg>
                Voltar
              </button>
            ...
            {step < 3 && (
              <button
                onClick={() => setStep((s) => s + 1)}
                type="button"
                style={{ display: "inline-flex", alignItems: "center", gap: 8, fontFamily: "inherit", fontSize: 14.5, fontWeight: 800, color: "#fff", background: PRIMARY, border: "none", borderRadius: 11, padding: "13px 24px", cursor: "pointer", boxShadow: "0 8px 20px rgba(204,31,31,0.26)", marginLeft: "auto" }}
              >
                Continuar
